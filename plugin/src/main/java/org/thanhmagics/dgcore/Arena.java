...
//        data.enc = EnumType.BLOCK;
    ...
    public void regenIfEmpty() {
        for (DGLocation dgLocation : data.region.keySet()) {
            if (!Bukkit.getWorld(dgLocation.w).getBlockAt(dgLocation.getLocation()).getType().equals(Material.AIR))
                return;
        }
        try {
            Bukkit.getScheduler().runTask(DGCore.getInstance(), () -> {
...
//            }
        ...
        if (i3 == 0 && i4 == 0)
            dgPlayer.level++;
        if (DGCore.getInstance().block_name_file != null) {
